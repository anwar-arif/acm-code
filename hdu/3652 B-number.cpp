...
int k , dp[13][2][3][13] , inp[13];

int rec(int pos,bool cmp,int sub,int mod){
    if(pos > k)return (sub==2 && mod==0);
    int &ret = dp[pos][cmp][sub][mod];
    if(ret != -1)return ret;

    int limit = cmp ? inp[pos] : 9;
    ret = 0;
    for(int i = 0 ; i <= limit;i++){
        int t = (sub==1 && i == 3)?2:(i==1)?1:0;
        if(sub == 2)t = 2;
        ret += rec(pos+1,cmp && i==limit,t,(mod*10 + i) % 13);
    }
    return ret;
}
