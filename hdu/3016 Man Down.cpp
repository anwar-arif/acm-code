...
int bfs(int s){
    ...
    while(!q.empty()){
        int u = q.front();q.pop();
        int cnt = 0;
        deb("node",u,"val",level[u]);
        for(int v = u+1;v <= n;v++){

            if(a[v].xl < a[u].xl and a[v].xr > a[u].xr){
                level[v] = level[u] + level[v];
                q.push(v);
                break;
            }
            if(a[v].xl <= a[u].xl and a[v].xr < a[u].xr){
            ...
            else if(a[v].xl > a[u].xl and a[v].xr > a[u].xr){
                ++cnt;
                level[v] += level[v];
                q.push(v);
                if(cnt == 2)break;
...
int main()
{
      ...
      {
          for(int i = 1; i<=n;i++){
              sc4i(a[i].h,a[i].xl,a[i].xr,a[i].val);
              level[i] = a[i].val;
          }
          int ans = 100 + bfs(1);
          if(ans > 0)pf("%d\n",ans);
