...
void update(int node,int b,int e,int indx){
    ...
    rt = lft + 1;
    mid = (b + e)/2;
    update(lft,b,mid,indx);
    update(rt,mid+1,e,indx);
...
int main()
{
      ...
      while(tst--){
          ...
          mem(tree,0);
          for(int i = 1; i<= m;i++){
              while(a[k].val <= q[i].h && k <= n){
                  update(1,1,n,a[k].indx);
                  ++k;
              }
              ans[q[i].indx] = query(1,1,n,q[i].l+1,q[i].r+1);
          }
          pf("Case %d:\n",++cas);
          for(int i = 1; i<= m;i++){
              pf("%d\n",ans[i]);
