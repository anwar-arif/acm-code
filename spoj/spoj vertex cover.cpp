...
int dp[SIZ][3],par[SIZ];
vector<int>g[SIZ];

int rec(int u,int isgard)
{
    int len = SZ(g[u]);
    if(len==0)return 0;
    if(dp[u][isgard] != -1)return dp[u][isgard];
    int sum = 0;
    for(int i = 0;i<len;i++){
        int v = g[u][i];
        if(v != par[u]){
            par[v] = u;
            if(isgard==0)sum+=rec(v,1);
            else sum+=min(rec(v,0),rec(v,1));
        }
    }
    return dp[u][isgard] = sum + isgard;
}
