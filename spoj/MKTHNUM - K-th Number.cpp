...
void build(int node,int b,int e){
    ...
    int l = SZ(mrg[lft]);int r = SZ(mrg[rt]);
    int i = 0,j = 0;
    while(i < l and j < r){
        if(mrg[lft][i] <= mrg[rt][j]){
            mrg[node].pb(mrg[lft][i]);
            i++;
        }
        else{
            mrg[node].pb(mrg[rt][j]);
            j++;
        }
    }
    while(i < l){
        mrg[node].pb(mrg[lft][i]); i++;
    }
    while(j < r){
        mrg[node].pb(mrg[rt][j]); j++;
    }
}
...
void query(int node,int b,int e,int i,int j){
    ...
    if(b >= i and e <= j){
        segment.pb(node);return;
    }
    int lft,rt,mid;
    lft = node << 1; rt = lft + 1;mid = (b+e)>>1;
    query(lft,b,mid,i,j);
    query(rt,mid+1,e,i,j);
}
...
int cal(int v){
    ...
    for(int i = 0; i< len;i++){
        int pos = segment[i];
        low = lower_bound(mrg[pos].begin(),mrg[pos].end(),v);
        int t = low - mrg[pos].begin();
        cnt += t;
        if(t < SZ(mrg[pos]) and mrg[pos][t] == v)cnt++;
    }
    return cnt;
}
