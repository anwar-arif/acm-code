...
int divisor[N+5] , status[(M/32)+10]  , k , primes[500];
...
bool isprime(int n){
    for(int i = 1; primes[i]*primes[i] <= n ; i++){
        if(n % primes[i] == 0 && (primes[i]*primes[i] != n))return false;
    }
    return true;
}
...
//    freopen("C:/Users/anwar_sust/Desktop/input.txt","r",stdin);
    ...
    sieve();
    for(int i = 3; i <= N; i++){
        for(int j = 1; primes[j]*primes[j] <= divisor[i] ; j++){
            if(isprime( divisor[i] / primes[j]) ){pf("%d\n",i);break;}
        }
    }
    return 0;
}
