...
void update(int node,int b,int e,int id){
    if(b > id or e < id)return;
    if(b >= id && e <= id){
        tree[node] = 1; return;
    }
    int lft,rt,mid;
    lft = node << 1; rt = lft + 1; mid = (b+e)/2;
    update(lft,b,mid,id); update(rt,mid+1,e,id);
...
int main()
{
      ...
      while(sc1i(n) != EOF){
          ...
          int p = 1;
          for(int i = 1; i<= m;i++){
              while(p <= n and a[p].val <= q[i].k){
                  update(1,1,n,a[p].indx);
                  ++p;
              }
              ans[q[i].indx] = (q[i].r - q[i].l + 1)-query(1,1,n,q[i].l,q[i].r);
          }
          for(int i = 1; i<= m;i++){
              pf("%d\n",ans[i]);
          }
