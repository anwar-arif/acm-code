...
void query(int node,int b,int e,int carry)
{
    if(b == e)
    {
        tree[node] = max(carry,tree[node]);
//        if(tree[node] == 0)tree[node]=carry;
        if(poster[tree[node]] == false)
        {
...
//    if(tree[node] == 0)tree[node] = carry;
    tree[node] = max(carry,tree[node]);
    query(lft,b,mid,tree[node]);
    query(rt,mid+1,e,tree[node]);
}
