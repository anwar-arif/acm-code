...
int level[N],par[N][25],t[N],maxi[N][25],mini[N][25],n;
    ...
    for(int j = 1;j < 22;j++){
        for(int i = 1; i<=n;i++){
            if(par[i][j-1] != -1){
                ...
                int p = par[i][j-1];
                maxi[i][j] = max(maxi[p][j-1],maxi[i][j-1]);
                mini[i][j] = min(mini[p][j-1],mini[i][j-1]);
            }
        }
    }
}
