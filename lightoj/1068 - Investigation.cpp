...
int rec(int pos,int rem,int remdigit,bool cmp)
{
    if(pos == len)return (rem+remdigit)==0?1:0;
    int &ret = dp[pos][rem][remdigit][cmp];
    if(ret != -1)return ret;
    int lim = cmp ? inp[pos] : 9;
    ll ans = 0;
    for(int i = 0 ; i <= lim;i++)
    {
        ans += rec(pos+1,(rem*10+i)%k,(remdigit+i)%k,cmp and i==lim);
    }
    return ret = ans;
}
