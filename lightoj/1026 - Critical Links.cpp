...
vector<int>g[N] , res;
stack<int>st;
int par[N] , fin[N] , dis[N] , depth , scc , ans;
int belong[N] , sz[N];

void reset(int x){
    repi(i , 0 , x + 2)g[i].clear() , par[i] = i;
    mem(fin,-1) ; mem(dis,-1);
    scc = depth = ans = 0;
...
int main()
{
    ...
    //Read;
    //Write;
//    freopen("C:/Users/Anwar/Desktop/input.txt","r",stdin);
    #endif
    int tst,cas = 0 ;
    int u , v;
    sc1i(tst);
    while(tst--){
        ...
        reset(n);
        repi(i ,  1, n){
            sc("%d %c%d%c",&u,&c1,&tot,&c2);getchar();
            for(int j = 1; j <= tot;j++){
                sc1i(v);
                g[u].pb(v);
                g[v].pb(u);
