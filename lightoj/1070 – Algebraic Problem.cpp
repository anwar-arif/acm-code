...
unsigned ll mod = (unsigned long long int)(1 << 64);
...
matrix multiply(matrix a, matrix b) {
  ...
  for (ull i = 1; i <= r.row; i++) {

    for (ull j = 1; j <= r.col; j++) {

      ull sum = 0;
      for (ull k = 1; k <= a.col;  k++) {
        sum += (a.v[i][k] * b.v[k][j]);
//        sum %= mod;
      }
//      sum += mod;
...
int main()
{
    ...
    mat.row = mat.col = 2;
    while(tst--){
        sc("%llu %llu %llu",&p,&q,&n);
        pf("Case %lld: ",++cas);

        if(n == 0)pf("2\n");
        else if(n == 1)pf("%llu\n",p);
        else{
            mat.v[1][1] = p;mat.v[1][2] = -q;
            mat.v[2][1] = 1; mat.v[2][2] = 0;

            matrix res = power(mat , n-1);

            ull ans = res.v[1][1]*p + res.v[1][2]*2;
//            ans %= mod;
            pf("%llu\n",ans);
        }
    }
