...
int n,dam[20][20],health[20],dp[(1<<20)+5];

int rec(int mask,int cnt)
{
    if(cnt == n)return 0;
    if(dp[mask] != -1)return dp[mask];
    int mn = INT_MAX,shoot,ret;
    for(int i = 0 ;i < n;i++)
    {
        if(check(mask,i) == 0)
        {
            for(int j = 0;j<=n;j++)
            {
                if(check(mask,j) != 0 and dam[j][i] != 0)
                {
                    shoot = (int)ceil((double)health[i]/(double)dam[j][i]);
                    on(mask,i);
                    mn = min(mn,shoot+rec(mask,cnt+1));
                    off(mask,i);
                }
            }
        }
