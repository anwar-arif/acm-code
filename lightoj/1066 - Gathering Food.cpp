...
{
    if(x <n and x >= 0 and y < n and  y >=0 and s[x][y] != '#' and !visited[x][y])return true;
    return false;
}
...
int bfs(int sx,int sy,int tx,int ty)
{
    ...
    while(!q.empty())
    {
        ...
        int x = p.first;
        int y = p.second;
        if(x == tx and y == ty)return level[x][y];
        for(int i = 0 ; i< 4;i++)
        {
            int xx = x + dx[i];
            int yy = y + dy[i];
            if(valid(xx,yy))
            {
                if(s[xx][yy] >= 'A' and s[xx][yy] <= 'Z' and ch >= s[xx][yy])
                {
                    visited[xx][yy] = true;
...
int main()
{
      ...
      while(tst--)
      {
          ...
          for(int i = 1;i < cnt;i++)
          {
              ...
              int y = a[i].y;
              char ch = s[x][y];
              int ret = bfs( a[i-1].x , a[i-1].y, x , y );
              if(ret == -1){
                t = false;break;
              }
              ans += ret;
