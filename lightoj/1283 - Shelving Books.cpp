...
int rec(int pos , int lo , int hi){
    int &ret = dp[pos][lo][hi];
    if(ret != -1)return ret;
    if(pos > n)return 0;

    ret = rec(pos+1,lo,hi);

    if(a[pos] >= a[lo] && a[pos] <= a[hi]){
        ret = max(ret , rec(pos+1 , pos , hi) + 1);
        ret = max(ret , rec(pos+1 , lo , pos) + 1);
    }
