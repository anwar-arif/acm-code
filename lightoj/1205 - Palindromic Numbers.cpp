...
ll rec(int pos , int cmp , int dig , string s){
//    deb("s=>",s);
    int l = SZ(s);
    if(pos == len)return isPal(s,l);
    ll &ret = dp[pos][cmp][dig];
    if(ret != -1)return ret;
...
//    deb("lim",limit);
    ...
    for(int i = 0 ; i <= limit ;i++){
        ret += rec(pos+1 , cmp && i==limit , i , (l+i == 0)?s : s+char('0'+i) );
    }
    return ret;
}
