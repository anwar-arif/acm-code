...
void dij(int s , int t){
    ...
    while( !q.empty() ){
        ...
        for(int i = 0 ;i < len ;i++){
            ...
            int tmp = level[u][op] + cost[u][i] ;

            if( tmp < level[v][1] ){
                level[v][2] = level[v][1] ;
                level[v][1] = tmp ;
                q.push( edge( v , level[v][1] , 1 ) ) ;
            }
            else if( tmp > level[v][1] && tmp < level[v][2] ){
                level[v][2] = tmp ;
                q.push( edge(v , level[v][2] , 2) ) ;
            }
        }
    }
}
...
void reset(){
    rep( i , 0 , N-2 ){
        g[i].clear() ;
        cost[i].clear();
        level[i][0] = level[i][1] = level[i][2] = inf ;
    }
}
