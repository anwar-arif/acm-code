...
dbl cal_area(dbl a,dbl b,dbl c)
{
    dbl s = (a+b+c)/2.0;
    s *= (s-a)*(s-b)*(s-c);
    s = sqrt(s);
...
dbl total_area,ab,ac,bc,r,ans;

void rec(dbl low,dbl high)
{
    if(high-low <= eps)return;
    dbl mid = (high+low)/2.0;
    dbl ae = (mid/ab)*ac;
    dbl de = (mid/ab)*bc;
    dbl a1 = cal_area(mid,ae,de);
    dbl a2 = total_area-a1;
    dbl cur_r = (a1/a2);
    if(cur_r==r){
        ans = mid;
        return;
