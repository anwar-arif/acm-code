...
matrix pwr( matrix mat , int p ) {
    if( p == 1 ) return mat ;
    if( p&1 ) {
        return mul(mat, pwr(mat, p - 1));
    }
    else{
        matrix ret = pwr(mat, p / 2);
        ret = mul(ret, ret);
        return ret;
    }
}
...
int main(){
    ...
    while( tst-- ) {
        ...
        if( n == 1 ) pf("1\n");
            ...
            mat.row = 4 ;
            mat.col = 4 ;
            rep( i , 1 , 4 ) rep( j , 1 , 4) mat.v[i][j] = 0 ;
            rep( i , 1 , 4) mat.v[1][i] = 1 ;
            ...
            mat = pwr( mat , n - 2 ) ;
            int ans = 2*mat.v[1][1] + mat.v[1][2] + mat.v[1][3] + mat.v[1][4] ;
            ans %= mod ;
            pf("%d\n", ans ) ;
        }
