...
ll cal(ll n,ll m)
{
    ...
    while(n != 1)
    {
        ...
        if(n%prime[i] == 0)
        {
            ...
            ll a = bigmod(prime[i],(cnt*m)+1,mod)-1;
            ll b = bigmod(prime[i]-1,mod-2,mod);

            ll x = (((a+mod)%mod) * ((b+mod)%mod))%mod;
            sum = (((sum+mod)%mod)*((x+mod)%mod))%mod;
        }
        i++;
    }
    if(n > 1)
    {
        ll a = bigmod(n,m+1,mod)-1;
        ll b = bigmod(n-1,mod-2,mod);
