...
int rec(int pos , int v , int c){
    ...
    if(dp[pos][v][c] != -1)return dp[pos][v][c];

    int ret;

    if(a[pos] == '?'){
        int p1 = (v >= 2) ? 1 : rec(pos+1,min(v+1,3),0);
        int p2 = (c >= 4) ? 1 : rec(pos+1,0 , min(c+1,5));
        ret = p1 | p2;
    }

    else if(is_vowel(a[pos])){
        ret = (v >= 2) ? 1 : rec(pos+1,min(v+1,3),0);
    }

    else ret = (c >= 4) ? 1 : rec(pos+1,0 , min(c+1,5));

    return dp[pos][v][c] = ret;
}
