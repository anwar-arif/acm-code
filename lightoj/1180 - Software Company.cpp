...
bool dp[N][N][N] ;
pii arr[N] ;
int vis[N][N][N] , ptr , mid , n , m , lo , hi , ans ;

bool rec( int pos , int r1 , int r2 ) {
    if( r1 + r2 == 0 ) return true ;
    if( pos > n ) return false ;
    bool &ret = dp[pos][r1][r2] ;
    int &v = vis[pos][r1][r2] ;
    if( v == ptr ) return ret ;
    v = ptr ;
    ret = false ;
    for( int i = 0 ; i <= r1 ; i++ ) {
        if( i * arr[pos].first > mid ) break ;
        int b = ( mid - (i*arr[pos].first) ) / arr[pos].second ;
        if( b >= r2 ) b = r2 ;
        ret |= rec( pos+1 , r1 - i , r2 - b ) ;
    }
    return ret ;
}
