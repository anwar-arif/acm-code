...
void rec(ll pos,ll s[],vector<ll>&v,ll n)
{
    ll len = (long long)SZ(res);
    if(len > 0){
        ll sum = 0;
        for(ll i = 0;i<len;i++){
            sum += res[i];
...
ll bs(ll val,vector<ll>& v)//finding upper bound for val
{
    ll len = (long long)SZ(v);
    ll low = 0,hi = len-1;
    ll ans = -1;
    while(low <= hi){
        ll mid = (low+hi)/2;
