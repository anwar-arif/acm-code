...
const int M = 1e8;
int primes[5761455+5] ;
int status[(M/32)+10] , k;
unsigned int mul[5761455+5] ;
...
int main(){
    ...
    mul[0] = 1;
    for(int i = 1;i <= k ;i++){
        mul[i] = mul[i-1] * unsigned(primes[i]);
    }
    int tst , cas = 0;
    sc1i(tst);
    while(tst--){
        sc1i(n);
        int up = upper_bound(primes+1 , primes+1+k , n) - primes - 1;
