...
bool okay(int x,int y){
    if(man[x].d != woman[y].d)return false;
    int a_gap = man[x].age - woman[y].age;
    if(a_gap < 0) a_gap *= -1;
    int h_gap = man[x].h - woman[y].h;
    if(h_gap < 0)h_gap *= -1;
    if(h_gap <= 12 and a_gap <= 5)return true;
    return false;
}
...
bool dfs(int u){
    ...
    for(int i = 0 ;i < len;i++){
        ...
        if(!visited[v]){
            visited[v] = true;
            if(match[v] == -1 or dfs(match[v]) == true){
                match[v] = u;
                return true;
            }
        }
...
int main()
{
      ...
      while(tst--){
          sc2i(m,w);
          for(int i = 1; i<=m;i++){
              sc3i(man[i].h,man[i].age,man[i].d);
          }
          for(int i = 1; i<= w;i++){
            sc3i(woman[i].h,woman[i].age,woman[i].d);
          }
