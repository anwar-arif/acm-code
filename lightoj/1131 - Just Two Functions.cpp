...
ll C1[4] , C2[4] , f[4] , g[4] , query[110];
...
int main(){
    ...
    sc1ll(tst);
    while(tst--){
        for(ll i = 1 ;i <= 3;i++)sc1ll(C1[i]);
        for(ll i = 1 ;i <= 3;i++)sc1ll(C2[i]);
        for(ll i = 1 ;i <= 3;i++)sc1ll(f[i]);
        for(ll i = 1 ;i <= 3;i++)sc1ll(g[i]);
...
//            C2[i] %= mod;
        ...
        for(ll i = 2; i <= 3;i++){
            for(ll j = 1; j <= 3;j++){
                mat1.v[i][j] = (j+1 == i) ? 1 : 0;
                mat2.v[i][j] = (j+1 == i) ? 1 : 0;
            }
        }
        matrix b1 ,b2,res;

        b1.row = b2.row = res.row = 3;
        b1.col = b2.col = res.col = 1;
        ...
        for(ll i = 1; i<= q;i++)sc1ll(query[i]);
