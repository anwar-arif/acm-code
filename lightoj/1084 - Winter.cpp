...
int rec( int pos ) {
    ...
    if( ret != -1 ) return ret ;
    ret = inf ;
    if( nxt[pos] - pos >= 3 )ret = min( ret , 1 + rec( nxt[pos] ) ) ;
    if( nxt[pos] - pos >= 4 )ret = min( ret , 1 + rec( nxt[pos] - 1 ) ) ;
    if( nxt[pos] - pos >= 5 )ret = min( ret , 1 + rec( nxt[pos] - 2 ) ) ;
    return ret ;
}
