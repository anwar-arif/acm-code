...
int rec(int prv,int mask){
    if(mask == (1 << n)-1 )return 0 ;
    int &ret = dp[prv][mask];
    if(ret != -1)return ret;
    ret = INT_MAX;
    repi(i , 0 , n-1){
        if((mask & r[i].mask) == r[i].mask && !check(mask,i)){
            ret = min(ret , rec( i , on(mask,i) ) + (r[prv].col != r[i].col) );
        }
    }
    return ret;
}
...
//    freopen("C:/Users/anwar_sust/Desktop/input.txt","r",stdin);
    ...
    while(tst--){
        sc1i(n);
        repi(i, 0, n-1){
            sc4i(r[i].y1 , r[i].x1 , r[i].y2 , r[i].x2);
            sc1i(r[i].col);
        }
        f();
        int ans = INT_MAX;
