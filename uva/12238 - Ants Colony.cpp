...
ll t[N],d[N],level[N],dist[N][21],par[N][21];
...
//        pf("---> %d\n",dist[i][0]);
    ...
    for(ll j = 1;j < 20;j++){
        for(ll i = 0 ; i < n;i++){
            if(par[i][j-1] != -1){
//                if(i==5 and j==2)deb(par[i][j-1] , dist[par[i][j-1]][j-1] , dist[i][j-1]);
                dist[i][j] = dist[ par[i][j-1] ][j - 1] + dist[i][j - 1];
//                if(i==5 and j==2)deb(dist[i][j]);
...
ll query(ll p,ll q){
    ...
    ll ans = 0;
//    deb("level p q",level[p],level[q]);
    for(ll i = 19; i>= 0;i--){
        if(level[p] - (1<<i) >= level[q]){
            ans += dist[p][i];
//            deb("dist ",i,"th",dist[p][i]);
            p = par[p][i];
//            deb("new p",p);
        }
    }
