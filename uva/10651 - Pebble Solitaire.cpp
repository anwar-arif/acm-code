...
int rec(int mask)
{
    ...
    {
        if(pos < 10 and check(mask,pos) and check(mask,pos+1) and !check(mask,pos+2))
        {
            int nmask = mask;
            off(nmask,pos);off(nmask,pos+1);on(nmask,pos+2);
            ret = min(ret,rec(nmask));
        }
        if(pos>1 and check(mask,pos) and check(mask,pos-1) and !check(mask,pos-2))
        {
            int nmask = mask;
            off(nmask,pos);off(nmask,pos-1);on(nmask,pos-2);
            ret = min(ret,rec(nmask));
