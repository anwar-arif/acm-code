...
//    #endif
    ...
    while( sc("%s" , &s ) != EOF ) {
        ...
        double mn = 10000000000000.0 ;
        sc("%lf %lf" , &rx , &ry ) ;
        for( int i = 0 ; i < n ; i++ ) {
            sc("%lf %lf" , &a[i].x , &a[i].y ) ;
            double d = sqrt( sqr(rx-a[i].x) + sqr(ry-a[i].y) ) ;
            mn = min( mn , d ) ;
        }
        for( int i = 1 ; i < n ; i++ ) {
            double d = distToLineSegment( PT(rx,ry) , PT(a[i-1].x , a[i-1].y) , PT(a[i].x,a[i].y));
            mn = min( mn , d ) ;
        }
        double d = distToLineSegment(PT(rx,ry) , PT(a[n-1].x , a[n-1].y) , PT(a[0].x , a[0].y)) ;
        mn = min( mn , d ) ;
        pf("%0.3lf\n" , mn ) ;
    }
    return 0;
