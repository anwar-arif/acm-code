...
int cas = 0 ;
unsigned ll dp[2][N][150] ;
int vis[2][N][150] , n , x ;

unsigned ll rec( int id , int pos , int s ) {
    if( pos > n ) {
        if( id == 0 ) return (s >= x) ;
        return ( s < x ) ;
    }
    int &v = vis[id][pos][s] ;
    unsigned ll &ret = dp[id][pos][s] ;
    if( v == cas ) return ret ;
    v = cas ;
    ret = 0 ;
    for( int i = 1 ; i <= 6 ; i++ ) {
        ret += rec( id , pos+1 , s+i ) ;
    }
    return ret ;
}
...
//    #endif
    ...
    while( sc2i( n , x ) != EOF ) {
        if( x == 0 && n == 0 ) break ;
        ++cas ;
        unsigned ll up = rec( 0 , 1 , 0 ) ;
        unsigned ll dwn = rec( 1 , 1 , 0 ) ;
        unsigned ll tot = up + dwn ;
        unsigned ll g = gcd( up , tot ) ;
        up /= g ;
        tot /= g ;
        if( up == 0 ) pf("0\n");
        else if( up == tot ) pf("1\n");
