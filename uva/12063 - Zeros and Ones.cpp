...
ll rec(ll zero , ll one , ll md){
    if(zero+one > n || zero>n/2 || one>n/2)return 0;
    ll &ret = dp[zero][one][md];
    if(ret != -1)return ret;
    ret = 0;
    ret += rec(zero+1,one,(md << 1)%k) + rec(zero,one+1,((md << 1)%k + 1)%k );
    return ret;
}
