...
ll rec(int en , int st , int mask ){
    if(mask == ((1 << tot) - 1 ) )return d[arr[st]][arr[en]];
    ll &ret = dp[en][st][mask] ;
    if(ret != -1)return ret ;
    ret = inf ;
    rep( i , 0 , tot-1 ){
        if(check(mask , i) == 0){
            ret = min( ret , rec(en , i , on(mask , i)) + d[arr[st]][arr[i]]);
        }
    }
    return ret;
}
