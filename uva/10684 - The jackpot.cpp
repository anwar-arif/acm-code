...
int main()
{
      while(sc1i(n) != EOF){
          ...
          int mx_so_far = 0 , mx_ending_here = 0;
          for(int i = 1; i<= n;i++){
              mx_ending_here += a[i];
              if(mx_ending_here < 0)mx_ending_here = 0;
              else if(mx_so_far < mx_ending_here)mx_so_far = mx_ending_here;
          }
          if(mx_so_far <= 0)pf("Losing streak.\n");
